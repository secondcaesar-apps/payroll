...
@Component({
  selector: 'app-loan-workflow',
  templateUrl: './loan-workflow.component.html',
  styleUrls: ['./loan-workflow.component.scss']
})
export class LoanWorkflowComponent implements OnInit {
  ...
  loadevent(){
    this.Api.Read(APIENUM.LOANWORKFLOWSETUP)
    .subscribe((res: any) => {
      this.loading = false;
  ...
  //  console.log(this.WorkFlow.value)
    ...
    this.Api.Create(APIENUM.LOANWORKFLOWSETUP, this.WorkFlow.value['Item']).subscribe((res: any) => {
